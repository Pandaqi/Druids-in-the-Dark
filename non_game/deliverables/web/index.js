...
var GodotAudio = {
  ...
  SampleNode: class SampleNode {
    ...
    /**
      ...
      /** @type {AudioBufferSourceNode["onended"]} */ this._onended = null;
...
function _godot_audio_init(p_mix_rate, p_latency, p_state_change, p_latency_update) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(52, 0, 1, p_mix_rate, p_latency, p_state_change, p_latency_update);
...
function _godot_audio_sample_register_stream(streamObjectIdStrPtr, framesPtr, framesTotal, loopModeStrPtr, loopBegin, loopEnd) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(66, 0, 1, streamObjectIdStrPtr, framesPtr, framesTotal, loopModeStrPtr, loopBegin, loopEnd);
...
function _godot_audio_sample_set_finished_callback(callbackPtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(67, 0, 1, callbackPtr);
...
function _godot_audio_sample_set_pause(playbackObjectIdStrPtr, pause) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(68, 0, 1, playbackObjectIdStrPtr, pause);
  const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr);
...
function _godot_audio_sample_set_volumes_linear(playbackObjectIdStrPtr, busesPtr, busesSize, volumesPtr, volumesSize) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 0, 1, playbackObjectIdStrPtr, busesPtr, busesSize, volumesPtr, volumesSize);
...
function _godot_audio_sample_start(playbackObjectIdStrPtr, streamObjectIdStrPtr, busIndex, offset, volumePtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(70, 0, 1, playbackObjectIdStrPtr, streamObjectIdStrPtr, busIndex, offset, volumePtr);
  /** @type {string} */ const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr);
...
function _godot_audio_sample_stop(playbackObjectIdStrPtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(71, 0, 1, playbackObjectIdStrPtr);
  const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr);
  GodotAudio.stop_sample(playbackObjectId);
}

function _godot_audio_sample_stream_is_registered(streamObjectIdStrPtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(72, 0, 1, streamObjectIdStrPtr);
  const streamObjectId = GodotRuntime.parseString(streamObjectIdStrPtr);
...
function _godot_audio_sample_unregister_stream(streamObjectIdStrPtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(73, 0, 1, streamObjectIdStrPtr);
  const streamObjectId = GodotRuntime.parseString(streamObjectIdStrPtr);
...
function _godot_audio_sample_update_pitch_scale(playbackObjectIdStrPtr, pitchScale) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(74, 0, 1, playbackObjectIdStrPtr, pitchScale);
  const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr);
...
function _godot_audio_worklet_create(channels) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(75, 0, 1, channels);
  try {
...
function _godot_audio_worklet_start(p_in_buf, p_in_size, p_out_buf, p_out_size, p_state) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(76, 0, 1, p_in_buf, p_in_size, p_out_buf, p_out_size, p_state);
...
function _godot_js_config_canvas_id_get(p_ptr, p_ptr_max) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(77, 0, 1, p_ptr, p_ptr_max);
  GodotRuntime.stringToHeap(`#${GodotConfig.canvas.id}`, p_ptr, p_ptr_max);
}

function _godot_js_config_locale_get(p_ptr, p_ptr_max) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(78, 0, 1, p_ptr, p_ptr_max);
...
function _godot_js_display_alert(p_text) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(79, 0, 1, p_text);
  window.alert(GodotRuntime.parseString(p_text));
}

// eslint-disable-line no-alert
function _godot_js_display_canvas_focus() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(80, 0, 1);
  GodotConfig.canvas.focus();
}
...
function _godot_js_display_clipboard_get(callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(82, 0, 1, callback);
...
function _godot_js_display_clipboard_set(p_text) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(83, 0, 1, p_text);
...
function _godot_js_display_cursor_lock_set(p_lock) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(86, 0, 1, p_lock);
  if (p_lock) {
...
function _godot_js_display_cursor_set_custom_shape(p_shape, p_ptr, p_len, p_hotspot_x, p_hotspot_y) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(87, 0, 1, p_shape, p_ptr, p_len, p_hotspot_x, p_hotspot_y);
...
function _godot_js_display_cursor_set_shape(p_string) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(88, 0, 1, p_string);
  GodotDisplayCursor.set_shape(GodotRuntime.parseString(p_string));
}

function _godot_js_display_cursor_set_visible(p_visible) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(89, 0, 1, p_visible);
...
function _godot_js_display_desired_size_set(width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(90, 0, 1, width, height);
  GodotDisplayScreen.desired_size = [ width, height ];
  GodotDisplayScreen.updateSize();
}

function _godot_js_display_fullscreen_cb(callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(91, 0, 1, callback);
  const canvas = GodotConfig.canvas;
  const func = GodotRuntime.get_func(callback);
...
function _godot_js_display_fullscreen_exit() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(92, 0, 1);
...
function _godot_js_display_has_webgl(p_version) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(94, 0, 1, p_version);
...
function _godot_js_display_notification_cb(callback, p_enter, p_exit, p_in, p_out) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(96, 0, 1, callback, p_enter, p_exit, p_in, p_out);
  const canvas = GodotConfig.canvas;
  const func = GodotRuntime.get_func(callback);
...
function _godot_js_display_screen_size_get(width, height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(99, 0, 1, width, height);
  const scale = GodotDisplayScreen.getPixelRatio();
...
function _godot_js_display_setup_canvas(p_width, p_height, p_fullscreen, p_hidpi) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(100, 0, 1, p_width, p_height, p_fullscreen, p_hidpi);
  const canvas = GodotConfig.canvas;
...
function _godot_js_display_vk_cb(p_input_cb) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(105, 0, 1, p_input_cb);
...
function _godot_js_display_vk_hide() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(106, 0, 1);
  GodotDisplayVK.hide();
}

function _godot_js_display_vk_show(p_text, p_type, p_start, p_end) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(107, 0, 1, p_text, p_type, p_start, p_end);
  const text = GodotRuntime.parseString(p_text);
...
function _godot_js_display_window_icon_set(p_ptr, p_len) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(109, 0, 1, p_ptr, p_len);
...
function _godot_js_display_window_size_get(p_width, p_height) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(110, 0, 1, p_width, p_height);
  GodotRuntime.setHeapValue(p_width, GodotConfig.canvas.width, "i32");
  GodotRuntime.setHeapValue(p_height, GodotConfig.canvas.height, "i32");
}

function _godot_js_display_window_title_set(p_data) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(111, 0, 1, p_data);
...
function _godot_js_fetch_create(p_method, p_url, p_headers, p_headers_size, p_body, p_body_size) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(112, 0, 1, p_method, p_url, p_headers, p_headers_size, p_body, p_body_size);
...
function _godot_js_fetch_free(id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(113, 0, 1, id);
  GodotFetch.free(id);
}

function _godot_js_fetch_http_status_get(p_id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(114, 0, 1, p_id);
...
function _godot_js_fetch_is_chunked(p_id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(115, 0, 1, p_id);
  const obj = IDHandler.get(p_id);
  if (!obj || !obj.response) {
    return -1;
  }
...
function _godot_js_fetch_read_chunk(p_id, p_buf, p_buf_size) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(116, 0, 1, p_id, p_buf, p_buf_size);
  const obj = IDHandler.get(p_id);
  if (!obj || !obj.response) {
    return 0;
  }
...
function _godot_js_fetch_read_headers(p_id, p_parse_cb, p_ref) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(117, 0, 1, p_id, p_parse_cb, p_ref);
  const obj = IDHandler.get(p_id);
  if (!obj || !obj.response) {
    return 1;
  }
...
function _godot_js_fetch_state_get(p_id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(118, 0, 1, p_id);
  const obj = IDHandler.get(p_id);
  if (!obj) {
    return -1;
  }
...
function _godot_js_input_gamepad_cb(change_cb) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(120, 0, 1, change_cb);
...
function _godot_js_input_gamepad_sample_count() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(122, 0, 1);
...
function _godot_js_input_gamepad_sample_get(p_index, r_btns, r_btns_num, r_axes, r_axes_num, r_standard) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(123, 0, 1, p_index, r_btns, r_btns_num, r_axes, r_axes_num, r_standard);
...
function _godot_js_input_key_cb(callback, code, key) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(124, 0, 1, callback, code, key);
  const func = GodotRuntime.get_func(callback);
...
function _godot_js_input_mouse_wheel_cb(callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(127, 0, 1, callback);
  const func = GodotRuntime.get_func(callback);
...
function _godot_js_input_paste_cb(callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(128, 0, 1, callback);
  const func = GodotRuntime.get_func(callback);
...
function _godot_js_input_touch_cb(callback, ids, coords) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(129, 0, 1, callback, ids, coords);
  const func = GodotRuntime.get_func(callback);
  const canvas = GodotConfig.canvas;
...
function _godot_js_input_vibrate_handheld(p_duration_ms) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(130, 0, 1, p_duration_ms);
...
function _godot_js_os_download_buffer(p_ptr, p_size, p_name, p_mime) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(132, 0, 1, p_ptr, p_size, p_name, p_mime);
...
function _godot_js_os_execute(p_json) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(133, 0, 1, p_json);
...
function _godot_js_os_finish_async(p_callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(134, 0, 1, p_callback);
  const func = GodotRuntime.get_func(p_callback);
...
function _godot_js_os_fs_sync(callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(136, 0, 1, callback);
  const func = GodotRuntime.get_func(callback);
...
function _godot_js_os_has_feature(p_ftr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(137, 0, 1, p_ftr);
...
function _godot_js_os_hw_concurrency_get() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(138, 0, 1);
...
function _godot_js_os_request_quit_cb(p_callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(139, 0, 1, p_callback);
  GodotOS.request_quit = GodotRuntime.get_func(p_callback);
}

function _godot_js_os_shell_open(p_uri) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(140, 0, 1, p_uri);
...
function _godot_js_pwa_cb(p_update_cb) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(141, 0, 1, p_update_cb);
...
function _godot_js_pwa_update() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(142, 0, 1);
...
function _godot_js_rtc_datachannel_connect(p_id, p_ref, p_on_open, p_on_message, p_on_error, p_on_close) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(144, 0, 1, p_id, p_ref, p_on_open, p_on_message, p_on_error, p_on_close);
...
function _godot_js_rtc_datachannel_send(p_id, p_buffer, p_length, p_raw) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(154, 0, 1, p_id, p_buffer, p_length, p_raw);
  const ref = IDHandler.get(p_id);
  if (!ref) {
    return 1;
  }
...
function _godot_js_rtc_pc_close(p_id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(155, 0, 1, p_id);
  const ref = IDHandler.get(p_id);
  if (!ref) {
    return;
  }
...
function _godot_js_rtc_pc_create(p_config, p_ref, p_on_connection_state_change, p_on_ice_gathering_state_change, p_on_signaling_state_change, p_on_ice_candidate, p_on_datachannel) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(156, 0, 1, p_config, p_ref, p_on_connection_state_change, p_on_ice_gathering_state_change, p_on_signaling_state_change, p_on_ice_candidate, p_on_datachannel);
...
function _godot_js_rtc_pc_datachannel_create(p_id, p_label, p_config) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(157, 0, 1, p_id, p_label, p_config);
  try {
    const ref = IDHandler.get(p_id);
    if (!ref) {
      return 0;
...
function _godot_js_rtc_pc_destroy(p_id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(158, 0, 1, p_id);
  GodotRTCPeerConnection.destroy(p_id);
}

function _godot_js_rtc_pc_ice_candidate_add(p_id, p_mid_name, p_mline_idx, p_sdp) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(159, 0, 1, p_id, p_mid_name, p_mline_idx, p_sdp);
  const ref = IDHandler.get(p_id);
  if (!ref) {
    return;
  }
...
function _godot_js_rtc_pc_local_description_set(p_id, p_type, p_sdp, p_obj, p_on_error) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(160, 0, 1, p_id, p_type, p_sdp, p_obj, p_on_error);
  const ref = IDHandler.get(p_id);
  if (!ref) {
    return;
  }
...
function _godot_js_rtc_pc_offer_create(p_id, p_obj, p_on_session, p_on_error) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(161, 0, 1, p_id, p_obj, p_on_session, p_on_error);
  const ref = IDHandler.get(p_id);
  if (!ref) {
    return;
  }
...
function _godot_js_rtc_pc_remote_description_set(p_id, p_type, p_sdp, p_obj, p_session_created, p_on_error) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(162, 0, 1, p_id, p_type, p_sdp, p_obj, p_session_created, p_on_error);
  const ref = IDHandler.get(p_id);
  if (!ref) {
    return;
  }
...
function _godot_js_set_ime_active(p_active) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(163, 0, 1, p_active);
  GodotIME.ime_active(p_active);
}

function _godot_js_set_ime_cb(p_ime_cb, p_key_cb, code, key) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(164, 0, 1, p_ime_cb, p_key_cb, code, key);
...
function _godot_js_set_ime_position(p_x, p_y) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(165, 0, 1, p_x, p_y);
  GodotIME.ime_position(p_x, p_y);
}

function _godot_js_tts_get_voices(p_callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(166, 0, 1, p_callback);
  const func = GodotRuntime.get_func(p_callback);
  try {
    const arr = [];
...
function _godot_js_tts_speak(p_text, p_voice, p_volume, p_pitch, p_rate, p_utterance_id, p_callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(171, 0, 1, p_text, p_voice, p_volume, p_pitch, p_rate, p_utterance_id, p_callback);
  const func = GodotRuntime.get_func(p_callback);
...
function _godot_js_tts_stop() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(172, 0, 1);
...
function _godot_js_websocket_buffered_amount(p_id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(173, 0, 1, p_id);
  return GodotWebSocket.bufferedAmount(p_id);
}

function _godot_js_websocket_close(p_id, p_code, p_reason) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(174, 0, 1, p_id, p_code, p_reason);
  const code = p_code;
...
function _godot_js_websocket_create(p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(175, 0, 1, p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close);
...
function _godot_js_websocket_destroy(p_id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(176, 0, 1, p_id);
  GodotWebSocket.destroy(p_id);
}

function _godot_js_websocket_send(p_id, p_buf, p_buf_len, p_raw) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(177, 0, 1, p_id, p_buf, p_buf_len, p_raw);
...
function _godot_js_wrapper_create_cb(p_ref, p_func) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(178, 0, 1, p_ref, p_func);
...
function _godot_js_wrapper_create_object(p_object, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(179, 0, 1, p_object, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback);
...
function _godot_js_wrapper_interface_get(p_name) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(180, 0, 1, p_name);
  const name = GodotRuntime.parseString(p_name);
  if (typeof (window[name]) !== "undefined") {
...
function _godot_js_wrapper_object_call(p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(181, 0, 1, p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback);
...
function _godot_js_wrapper_object_get(p_id, p_exchange, p_prop) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(182, 0, 1, p_id, p_exchange, p_prop);
  const obj = GodotJSWrapper.get_proxied_value(p_id);
  if (obj === undefined) {
    return 0;
  }
...
function _godot_js_wrapper_object_getvar(p_id, p_type, p_exchange) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(183, 0, 1, p_id, p_type, p_exchange);
  const obj = GodotJSWrapper.get_proxied_value(p_id);
  if (obj === undefined) {
    return -1;
  }
...
function _godot_js_wrapper_object_set(p_id, p_name, p_type, p_exchange) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(184, 0, 1, p_id, p_name, p_type, p_exchange);
  const obj = GodotJSWrapper.get_proxied_value(p_id);
  if (obj === undefined) {
    return;
  }
...
function _godot_js_wrapper_object_set_cb_ret(p_val_type, p_val_ex) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(185, 0, 1, p_val_type, p_val_ex);
...
function _godot_js_wrapper_object_setvar(p_id, p_key_type, p_key_ex, p_val_type, p_val_ex) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(186, 0, 1, p_id, p_key_type, p_key_ex, p_val_type, p_val_ex);
  const obj = GodotJSWrapper.get_proxied_value(p_id);
  if (obj === undefined) {
    return -1;
  }
...
function _godot_js_wrapper_object_unref(p_id) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(187, 0, 1, p_id);
...
function _godot_webgl2_glFramebufferTextureMultisampleMultiviewOVR(target, attachment, texture, level, samples, base_view_index, num_views) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(188, 0, 1, target, attachment, texture, level, samples, base_view_index, num_views);
...
function _godot_webgl2_glFramebufferTextureMultiviewOVR(target, attachment, texture, level, base_view_index, num_views) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(189, 0, 1, target, attachment, texture, level, base_view_index, num_views);
  const context = GL.currentContext;
...
function _godot_webgl2_glGetBufferSubData(target, offset, size, data) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(190, 0, 1, target, offset, size, data);
  const gl_context_handle = _emscripten_webgl_get_current_context();
  const gl = GL.getContext(gl_context_handle);
  if (gl) {
...
function _godot_webxr_get_bounds_geometry(r_points) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(191, 0, 1, r_points);
...
function _godot_webxr_get_depth_texture() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(193, 0, 1);
  const subimage = GodotWebXR.getSubImage();
  if (subimage === null) {
    return 0;
  }
...
function _godot_webxr_get_frame_rate() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(194, 0, 1);
...
function _godot_webxr_get_render_target_size(r_size) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(196, 0, 1, r_size);
  const subimage = GodotWebXR.getSubImage();
  if (subimage === null) {
    return false;
  }
...
function _godot_webxr_get_supported_frame_rates(r_frame_rates) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(197, 0, 1, r_frame_rates);
...
function _godot_webxr_get_transform_for_view(p_view, r_transform) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(198, 0, 1, p_view, r_transform);
  if (!GodotWebXR.session || !GodotWebXR.pose) {
    return false;
  }
...
function _godot_webxr_get_view_count() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(200, 0, 1);
  if (!GodotWebXR.session || !GodotWebXR.pose) {
    return 1;
  }
...
var _godot_webxr_initialize = function(p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(202, 0, 1, p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event);
...
function _godot_webxr_is_session_supported(p_session_mode, p_callback) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(203, 0, 1, p_session_mode, p_callback);
  const session_mode = GodotRuntime.parseString(p_session_mode);
...
function _godot_webxr_is_supported() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(204, 0, 1);
  return !!navigator.xr;
}

var _godot_webxr_uninitialize = function() {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(205, 0, 1);
...
function _godot_webxr_update_input_source(p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes, r_has_hand_data, r_hand_joints, r_hand_radii) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(206, 0, 1, p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes, r_has_hand_data, r_hand_joints, r_hand_radii);
...
function _godot_webxr_update_target_frame_rate(p_frame_rate) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(207, 0, 1, p_frame_rate);
